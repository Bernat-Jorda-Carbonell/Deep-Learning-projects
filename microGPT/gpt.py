...
class CausalSelfAttention(nn.Module):
    ...
    def apply_rotary_emb(self, xq: torch.Tensor, xk: torch.Tensor, T: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        freqs = torch.einsum("t,d->td", seq_pos, self.inv_freq.to(xq.device))   # Shape: (T, dim // 2) not freqs, but thetas
        pos_emb = torch.cat((freqs.sin(), freqs.cos()), dim=-1)  # Shape: (T, dim)

        pos_emb = pos_emb.unsqueeze(0).unsqueeze(0)  # Shape: (1, 1, T, dim)
        
        # Split pos into sin and cos components, repeating each to match xq and xk dimensions
        pos_sin = pos_emb[..., :dim//2] # Shape: (1, 1, T, dim // 2)
        pos_cos = pos_emb[..., dim//2:] # Shape: (1, 1, T, dim // 2)
        ...
        xq_rot_even = (xq[..., ::2] * pos_cos) - (xq[..., 1::2] * pos_sin)
        xq_rot_odd = (xq[..., ::2] * pos_sin) + (xq[..., 1::2] * pos_cos)
        xq_rot = torch.stack((xq_rot_even, xq_rot_odd), dim=-1).flatten(-2)

        xk_rot_even = (xk[..., ::2] * pos_cos) - (xk[..., 1::2] * pos_sin)
        xk_rot_odd = (xk[..., ::2] * pos_sin) + (xk[..., 1::2] * pos_cos)
        xk_rot = torch.stack((xk_rot_even, xk_rot_odd), dim=-1).flatten(-2)

        # xk_rot = (xk[..., ::2] * pos_cos) - (xk[..., 1::2] * pos_sin)
        
        
        # xq = torch.stack((xq_rot, xq[..., ::2] * pos_sin + xq[..., 1::2] * pos_cos), dim=-1).flatten(-2)
        # xk = torch.stack((xk_rot, xk[..., ::2] * pos_sin + xk[..., 1::2] * pos_cos), dim=-1).flatten(-2)
    ...
    def forward(self, x):
        ...
        q, k ,v  = self.c_attn(x).view(B, T, self.n_head, 3, C // self.n_head).permute(3, 0, 2, 1, 4) # (3, B, nh, T, hs)

        # print ("batch size, sequence length, embedding dimensionality", B, T, C , "q, k, v", q.shape, k.shape, v.shape)


        # q = q.permute(0,2,1,3).contiguous().view(B * self.n_head, T, C // self.n_head) # Queries
        # k = k.permute(0,2,1,3).contiguous().view(B * self.n_head, T, C // self.n_head) # Keys
        # v = v.permute(0,2,1,3).contiguous().view(B * self.n_head, T, C // self.n_head) # Values
        ...
        causal_mask = self.mask[:, :, :T, :T]  # Restrict mask to sequence length
        ...
        if self.use_flash_attn:
            try:
                ...
                y = F.scaled_dot_product_attention(q, k, v,
                                                ...
                                                is_causal=(self.mask is None))
            except Exception as e:
                print(f"Error in FlashAttention: {e}")
                raise
        else:
            # Compute attention scores
            att = (q @ k.transpose(-2, -1)) / math.sqrt(k.size(-1)) # (B, nh, T, hs) x (B, nh, hs, T) -> (B, nh, T, T)
            # Apply causal mask
            att = att.masked_fill(causal_mask == 0, -torch.inf) # Set masked positions to -inf 
            att = torch.softmax(att, dim=-1) # Softmax the attention weights
            att = self.attn_dropout(att) # Apply dropout to the attention weights
...
class TransformerDecoderBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Initialize Layer Normalization for input to Self-Attention
        self.layer_norm_1 = RMSNorm(config.n_embd)
        # Causal Self-Attention layer
        self.self_attention = CausalSelfAttention(config)
        # Initialize Layer Normalization for input to MLP
        self.layer_norm_2 = RMSNorm(config.n_embd)
        # Feed-Forward MLP
        self.mlp = nn.Sequential(
            nn.Linear(config.n_embd, 4 * config.n_embd),  # Expand dimensions
            BERTGELU(),  # GELU activation
            nn.Linear(4 * config.n_embd, config.n_embd),  # Project back to original dimensions
    ...
    def forward(self, x):
        ...
        attn_out = self.self_attention(self.layer_norm_1(x))  # Apply self-attention to normalized input
        x = x + attn_out # Residual connection
        
        # Apply LayerNorm and MLP
        mlp_out = self.mlp(self.layer_norm_2(x))  # Apply MLP to normalized input
        out = x + mlp_out  # Residual connection
        return out
...
class GPT(nn.Module):
    ...
    def forward(self, idx: torch.Tensor):
        ...
        for block in self.transformer.h:
            x = block(x)  # Apply each TransformerDecoderBlock
    ...
    def generate(self, idx: torch.LongTensor, max_new_tokens: int, temperature:float = 1.0, do_sample:bool = False, top_k:int = None, top_p: float = 0.6):
        ...
        for _ in range(max_new_tokens):
            ...
            logits = logits[:, -1, :]  
            if temperature <= 1.e-6:
                raise ValueError("Temperature value " + str(temperature) + " is too low. Must be greater than 1e-6.")
            logits = logits / temperature  

            if not do_sample:
                # take the most likely token AKA greedy
                idx_next = torch.argmax(logits, dim=-1, keepdim=True)
            
            else:
                # apply softmax to convert logits to (normalized) probabilities

                # logits = logits.clamp(min=-100, max=100)   # clamp the logits to avoid NaNs

                probs = torch.softmax(logits, dim=-1) 
                #print(f"logits (before softmax): {logits}")  
                #print(f"probs (after softmax): {probs}")
                ...
                if top_k is not None:
                    ...
                    #     print("Warning: Probabilities sum to zero after top-k filtering!")
                    #     print(f"logits (before softmax): {logits}")  
                    #     probs_sum = torch.ones_like(probs_sum)  # Prevent division by zero   
                    # probs = probs / probs_sum
                
                    #print(f"Filtered probs after top-k: {probs}")
                ...
                if top_p is not None:
                    ...
                    # probs_sum = probs.sum(dim=-1, keepdim=True)
                    # if torch.any(probs_sum == 0):
                    #     print("Warning: Probabilities sum to zero after top-p filtering!")
                    #     print(f"logits (before softmax): {logits}")  
                    #     probs_sum = torch.ones_like(probs_sum)  # Prevent division by zero   
                    # probs = probs / probs_sum
                    # #print(f"Filtered probs after top-p: {probs}")
